    ...
    tbody.innerHTML = recentStudents.map(student => {
        ...
        return `
            <tr>
                ...
                <td>${deptName}</td>
                <td>Year ${student.current_year || 'N/A'}</td>
                <td><span class="badge status-${student.enrollment_status || 'active'}">${student.enrollment_status || 'active'}</span></td>
            </tr>
        `;
    }).join('');
}
    ...
    tbody.innerHTML = studentsToDisplay.map(student => {
        ...
        return `
            <tr>
                ...
                <td><span class="badge status-${student.enrollment_status}">${student.enrollment_status}</span></td>
                <td>
                    <button class="btn btn-sm btn-info btn-action" onclick="viewStudent(${student.id})"><i class="bi bi-eye"></i></button>
                    <button class="btn btn-sm btn-warning btn-action" onclick="editStudent(${student.id})"><i class="bi bi-pencil"></i></button>
                    <button class="btn btn-sm btn-danger btn-action" onclick="deleteStudent(${student.id})"><i class="bi bi-trash"></i></button>
                </td>
            </tr>
        `;
    }).join('');
...
async function saveStudent() {
    ...
    const studentData = {
        ...
        current_year: parseInt(formData.get('year')),
        enrollment_status: formData.get('status') || 'enrolled'
    };
    
    try {
        if (currentStudentId) {
            // Update existing student
            await apiCall(`${API_ENDPOINTS.students}${currentStudentId}/`, 'PUT', studentData);
...
async function editStudent(studentId) {
    ...
    form.elements['year'].value = student.current_year || '';
    ...
    form.elements['status'].value = student.enrollment_status;
...
function filterStudents() {
    ...
    const filtered = students.filter(student => {
        ...
        const matchesYear = yearFilter === '' || student.current_year === parseInt(yearFilter);
